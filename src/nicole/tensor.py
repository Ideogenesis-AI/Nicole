...
class Tensor:
    ...
    def regularize(self) -> None:
        ...
        For higher-order non-Abelian tensors, blocks whose Bridge weight is a
        single scalar with ``abs(w) < torch.finfo(torch.float32).eps`` are
        normalised to 1: the weight is set to 1 and the factor is absorbed into
        ``R``.  Blocks with more than one weight value, or whose weight exceeds
        single-precision epsilon, are left unchanged.
        ...
        if len(self.indices) == 2:
            ...
            for key, arr in self.data.items():
                ...
                factor = bridge.weights[0, 0] / target
                self.data[key] = arr * factor
                bridge.weights[0, 0] = target
        else:
            # Higher-order: normalise single-scalar weights to 1
            for key, arr in self.data.items():
                bridge = self.intw.get(key)
                if bridge is None or bridge.weights.numel() != 1:
                    continue

                w = bridge.weights[0, 0]
                if w.abs() > _sp_eps:
                    continue

                self.data[key] = arr * w
                bridge.weights[0, 0] = torch.ones(1, dtype=self.dtype)
